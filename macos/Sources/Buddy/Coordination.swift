...
final class Coordination {
    // Per-install shared secret, generated by setup.sh. Deliberately no
    // built-in fallback: no secret file means no coordination at all, so a
    // default value can never leak into someone's install.
    static let secret: String = {
        let url = BuddyPaths.home.appendingPathComponent("secret")
    ...
    private func start(listenPort: UInt16) {
        ...
        if Coordination.secret.isEmpty {
            buddyLog("coord: no secret file in \(BuddyPaths.home.path), coordination disabled (run setup.sh)")
            return
        }
